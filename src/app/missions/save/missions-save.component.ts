...
@Component({
  selector: 'app-missions-save',
  imports: [CommonModule, NgSelectModule, FormsModule, RouterLink],
  templateUrl: './missions-save.component.html',
  styleUrl: './missions-save.component.scss'
})
export class MissionsSaveComponent {
  ...
  cardWritingStageIndex: number = -1
  isEditEnabled = false
...
width=${width},height=${height},left=${left},top=${top}`;
  ...
  setMissionProduct(item: MissionProduct, i: number) {
    let product = this.products.items.find(i => i.id == item.productId);
    let mp = this.mission.stockInvoice.items.filter(i => i.productId == item.productId);
    if (mp.length > 1) {
      this.toastr.error('الصنف موجود', 'خطأ')
      setTimeout(() => {
        this.mission.stockInvoice.items[i].productId = ''
      }, 100);
  ...
  */
  ...
  setContractService(item: MissionService, contract: MissionServiceContract, i: number, setUnitPrice: boolean) {
    let mp = contract.services.filter(i => i.serviceId == item.serviceId);
    if (mp.length > 1) {
      this.toastr.error('الخدمة موجودة', 'خطأ')
      setTimeout(() => {
        contract.services[i].serviceId = ''
      }, 100);
  ...
  saveMission() {
    ...
    if (mission.taskStateId == '50001001' && mission.contracts.length > 0) {
      mission.taskStateId = '50001002'
      updateDate = true
    }
    this.mservice.saveMission(this.mission, updateDate).subscribe({
      next: (v: any) => {
        this.saveFollowup(updateDate);
  ...
  saveFollowup(updateDate:boolean) {
    let mission = this.mission
    let user_id = '';
    ...
    const nextStage = mission.stages
    ...
    if (mission.nextStage_stage_id == 'BD9BB762-4ECA-404D-ADCE-CE9BECC9FAE6') {
      mission.stages[this.cardWritingStageIndex].assignedToId = user_id
      this.mservice.saveFollowupMission(mission, updateDate).subscribe({
        next: (v: any) => {
          this.toastr.success('تم تحديث بطاقة العمل', 'تمت العملية')
          this.router.navigate(['/missions-list'])
        },
  ...
  private getMissionById(id: any) {
    this.mservice.getMissionById(id).subscribe({
      next: (v: any) => {
        ...
        if (mission.contracts.length == 0 && mission.taskStateId == "50001001" &&
          mission.nextStage_stage_id == "BD9BB762-4ECA-404D-ADCE-CE9BECC9FAE6" &&
          mission.nextStage_status == "i"
        ) {
          contractorId = mission.stages.find(i => i.stageId === '604C9FF8-C962-464E-AFEE-3E513FC6E6D2')?.assignedToId
          if (contractorId) {
            mission.contracts.push(createMissionServiceContract({
              price: 0, additions: 0, deductions: 0, total: 0, services: [{}],
              contractorId: contractorId
            }))
          }

          contractorId = mission.stages.find(i => i.stageId === '50AB1C5D-1545-4557-82CF-F78A3BD597CA')?.assignedToId
          if (contractorId) {
            mission.contracts.push(createMissionServiceContract({
              price: 0, additions: 0, deductions: 0, total: 0, services: [{}],
              contractorId: contractorId
        ...
        this.cardWritingStageIndex = mission.stages.findIndex(i => i.stageId === 'BD9BB762-4ECA-404D-ADCE-CE9BECC9FAE6')
  ...
  getUserRoles() {
    this.authService.getUserRoles().subscribe({
      next: (v: any) => {
        this.userRoles = v;
        this.isEditEnabled = this.hasRole('admin') || (this.mission.taskStateId == '50001001' && this.hasRole('Card Writing Works'));
      },
      error: (e) => console.log(e.error),
    });
  }
