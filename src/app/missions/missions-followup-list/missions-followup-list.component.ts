...
import { Stage, StageStatus } from '../../models/MissionStage';
import { User } from '../../models/User';


@Component({
  selector: 'app-missions-followup-list',
  imports: [CommonModule, FormsModule, RouterLink, NgbModule],
  templateUrl: './missions-followup-list.component.html',
  styleUrl: './missions-followup-list.component.scss'
})
export class MissionsFollowupListComponent {
  ...
  localStorage_parameters = 'missions_followup_list_parameters'
  ...
  parameters: MissionParameters = { workTypeId: '', taskStateId: '', pageNumber: 1, pageSize: 20 }
  missions: ListResponse<Mission> = { length: 0, items: [] };
  users: ListResponse<User> = { length: 0, items: [] };
  stages: ListResponse<Stage> = { length: 0, items: [] };
...
width=${width},height=${height},left=${left},top=${top}`;
  ...
  getStages() {
    ...
    this.mservice.getStages({ pageNumber: 1, pageSize: 200 }).subscribe({
      next: (v: any) => {
        //console.log(v);
        this.progressStatus.getMissions = 103
        this.stages = { length: 0, items: v };
  ...
  getUsers() {
    ...
    this.mservice.getUsers({ pageNumber: 1, pageSize: 200 }).subscribe({
      next: (v: any) => {
        //console.log(v);
        this.progressStatus.getMissions = 103
    ...
    /* else {
      ...
      for (const item of this.user) {
        if (item.includes("nameidentifier")) {
          this.parameters.nextStage_assignedTo_id = item.split(": ")[1];
          break;
        }
      }
      //this.users = { length: 0, items: [{ id: this.parameters.nextStage_assignedTo_id, name: this.parameters.nextStage_assignedTo_id }] }
  ...
  getMissions(resetPageNo: boolean) {
    ...
    this.progressStatus.getMissions = 102
    this.mservice.getFollowupMissions(this.parameters).subscribe({
      next: (v: any) => {
        //console.log(v);
        this.progressStatus.getMissions = 103
  ...
  getMissionsCount() {
    this.progressStatus.getMissions = 102
    this.mservice.getFollowupMissionsCount(this.parameters).subscribe({
      next: (v: any) => {
        //console.log(v);
        this.progressStatus.getMissions = 103
